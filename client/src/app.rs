...
pub fn initialize() {
    log::trace!("[regames_cef] app::initialize()");
    log::trace!("[regames_cef] App::new() ->");
    ...
    if client_api::samp::version::is_unknown_version() {
        log::error!("Версия не поддерживается [regames_cef]");

        client_api::utils::error_message_box(
                "[regames_cef] Версия не подходит под клиент RE-GAMES",
                "Пожалуйста сделайте проверку файлов клиента игры.\n С уважением, RE-GAMES COMPANY ltd.",
            );
